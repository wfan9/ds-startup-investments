...
download.file("https://raw.githubusercontent.com/wfan9/ds-startup-investments/master/references.bib", dl)
```
...
The `market` variable is a string representing the main market the startup is targeting.
...
The variable `name`, may be some sort of predictive power here. For example, a catchy or memorable startup
name might attract more attention and lead to more investment, consumer interest etc. However in terms of this
...
There are "startups" that were founded in the 1600's. Companies that old are not likely to be considered
...
data[1, c("name", "funding_total_usd", "seed", "venture")] %>% kable(caption = "Funding Sample")
```
...
In addition, there are values consisting of a single dash character indicating a missing value. Rows with
...
At the city level there is also stronger variation than at the region, but using this is likely too fine
grained and lead to overfitting.

### Funding

Box plot of funding against status shows startups with acquired status tend to have more funding, and those
with closed status slightly less funding.

```{r status-funding-box-plot, include = TRUE}
data %>% group_by(status) %>%
...
```{r status-funding-freq-plot, include = TRUE}
data %>% group_by(status) %>%
...
Most of the funding based features had zero variation and of no use, the two used are `funding_total_usd` and
`funding_rounds`.
...
help the building and evaluation of models (15%), and a test set that is only used to test final model accuracy
...
```{r multinom-orig-cm, include = TRUE}
multinom_cm
```
...
```{r multinom-smote-cm, include = TRUE}
smoted_multinom_cm
```
...
In this project, a multinomial logistic classification model was created to predict the status of startups,
