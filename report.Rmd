...
There are `r unique(data$market)` unique values for market, including NA values which will be replaced with
the string "Unknown".
...
```{r market-status-proportion-plot}
status_percentages <- market_data %>% group_by(market) %>%
  mutate(n_market = n()) %>%
...
rm(market_data, status_percentages)
```
...
Looking at the data shows there are 6 rows where there is truncation, however these will be left as they are
since there are only a few.

The `category_list` field needs to be split into individual categories to look for status - category relationships.
...
There are `r n_distinct(category_data$category)` categories. Creating plots similar to those for `market`:

```{r category-status-plot}
min_count <- 750
...
```{r category-status-proportion-plot}
status_percentages <- cat_data %>% group_by(category) %>%
  mutate(n_category = n()) %>%
...
#rm(category_data, cat_data, status_percentages)
```
